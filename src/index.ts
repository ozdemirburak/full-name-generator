import { names } from './names-lite';
import { surnames } from './surnames-lite';
...
type NameGenerator = (isoCode: string, gender: Gender) => string;
type SurnameGenerator = (isoCode: string, gender?: Gender) => string;

const generateMultiple = (
  generator: NameGenerator | SurnameGenerator,
  isoCode: string,
  gender: Gender | undefined,
  n: number
): string => {
  return Array.from({ length: n }, () => generator(isoCode, gender as Gender)).join(' ');
};
